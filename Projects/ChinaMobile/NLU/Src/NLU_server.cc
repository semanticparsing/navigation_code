...
DEFINE_int32(port, 20917, "serving port");
...
int main(int argc, char** argv) {
    ...
    boost::shared_ptr<chinaMobile::NLUServingHandler> handler(new chinaMobile::NLUServingHandler());
    if ( handler.get() == NULL ) {
        return -1;
    }
    if ( !handler->Init() ) {
    ...
    boost::shared_ptr<TProcessor> processor( new chinaMobile::NLUServingProcessor(handler) );
