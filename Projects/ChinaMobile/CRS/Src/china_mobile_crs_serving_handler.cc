...
void CChinaMobileCRSServingHandler::ChinaMobileCRSProcess(ChinaMobileBundle &response,
...
bool CChinaMobileCRSServingHandler::Process(const ChinaMobileBundle &request, ChinaMobileBundle &response)
{
    ...
    // 获取一个当前session对应的CoreAgentInfo
    // 如果是一个新的session，则新建一个coreAgenInfo并插入到_session_2_core和_sessions中
    // 如果是一个已经存在的session则返回对应的CoreAgentInfo
    // 正常情况下，is_insert_session应该为false
    // 只有当两个不同的对话使用相同的sessionId时则为true
    // 当is_insert_session为true，则当前对话处理完成后不能在_session中清除当前session，而是应该由当初插入sessionId的对话完成清理工作
    TCoreAgentInfo core_agent_info = GetCoreAgent(session, is_insert_session);

    // 初始化TCoreAgentInfo失败，如果是一个新生成的TCoreAgentInfo则清除该TCoreAgentInfo
    ...
    else if (ret) // 非挂机操作，将当前session对应的状态信息（ChinaMobileSession）保存到redis中
    {
#ifdef PERSISTENCE_REDIS
...
bool CChinaMobileCRSServingHandler::DialogTaskRun(const std::string &session,
                                                  const ChinaMobileBundle &request,
                                                  TCoreAgentInfo &core_agent_info,
                                                  ChinaMobileBundle &response)
{
    if (core_agent_info.pCoreAgent == NULL)
    {
    ...
    else if (request.inaction == _inaction_call_ret)
    { // 交互
        ...
        // 挂断结束或系统异常，设置mobileSession的结束原因，然后结束任务树执行
        if (request.inparams.input == _input_hangup || request.inparams.input == _input_sys_err)
        {
            ...
            if (mobile_session != NULL)
            {
                if (request.inparams.input == _input_hangup)
                { // 用户挂机
                    mobile_session->SetCloseReason("6");
                }
                else if (request.inparams.input == _input_sys_err)
                { // 系统异常挂断
...
TCoreAgentInfo CChinaMobileCRSServingHandler::GetCoreAgent(const std::string &session,
                                                           bool &is_insert_session)
{
    ...
    do
    {
        ...
        // 不能同时处理两个相同的 session
        // 正常情况下，不同的对话应该使用不同的sessionId，但是为了防止sessionId之间出现重复，增加了该机制
...
bool CChinaMobileCRSServingHandler::LoadCRSFromRedis(TCoreAgentInfo &core_agent_info, const std::string &session)
{
    ...
    */
    ...
    if (root != NULL)
    {
        ...
        {
            VLOG(WARNING) << __FUNCTION__ << ", Root Load Fail, Agents.size(): " << agents.size() << ", concepts.size(): " << concepts.size();
            CAL_TIME
            return false;
        }
    }
...
void CChinaMobileCRSServingHandler::CheckReleaseCoreAgent()
{
    ...
    //当前保存的session已经到达上限，需要删除旧的session
    int del_count = 0;
