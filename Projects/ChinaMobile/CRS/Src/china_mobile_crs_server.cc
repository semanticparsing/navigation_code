...
DEFINE_int32(port, 20919, "serving port");
DEFINE_int32(threads, 240, "serving threads");
...
int main(int argc, char** argv) {
    ...
    boost::shared_ptr<chinaMobile::CChinaMobileCRSServingHandler> handler(new chinaMobile::CChinaMobileCRSServingHandler());
    if ( handler.get() == NULL ) {
        return -1;
    }
    if ( !handler->Init() ) {
    ...
    boost::shared_ptr<TProcessor> processor( new chinaMobile::ChinaMobileCRSServingProcessor(handler) );
