...
DEFINE_int32(port, 201918, "serving port");
...
int main(int argc, char** argv) {
    ...
    boost::shared_ptr<robot::QueryParserServingHandler> handler(new robot::QueryParserServingHandler());
    if ( handler.get() == NULL ) {
        return -1;
    }
    if ( !handler->Init() ) {
    ...
    boost::shared_ptr<TProcessor> processor( new robot::QueryParserServingProcessor(handler) );
