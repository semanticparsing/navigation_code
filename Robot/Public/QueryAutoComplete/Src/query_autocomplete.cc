...
DEFINE_string(segmenter_data_dir, "../Data/WordSegTag/data/",
    "segmenter data directory");
...
bool QueryAutoComplete::Init() {
  ...
  segmenter_ = new trio::WordSegTag(FLAGS_segmenter_data_dir);
  if (segmenter_ == NULL) {
    VLOG(1) << "init segmenter failed.";
    return false;
  }
  segmenter_->Init();
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    complete_query_[items[0]] = make_pair(items[1], CompleteType(StringToInt(items[2])));
  }
...
bool QueryAutoComplete::GetReFromFile(const string& re_file,
...
bool QueryAutoComplete::GetRe2FromFile(const string& re_file,
                                      string* re_expression) {
  if (re_file.empty()) {
    VLOG(2) << "regular file is empty.";
    return false;
...
bool QueryAutoComplete::ReadFileToSet(const string& file_path,
                                      set<string>* term_set) {
  if (file_path.empty()) {
...
void QueryAutoComplete::GetKeySentence(const string &text,
                                       string* sentence) {
...
void QueryAutoComplete::SeperateSentences(const string &text,
    vector<pair<string, string> >* sents) {
...
void QueryAutoComplete::PronounReverse(const string& sentence,
                                       string* sentence_reverse) {
  sentence_reverse->clear();
  string segment_result_str(""); 
  segmenter_->Segment(sentence, segment_result_str);
  vector<string> segs_vec;
  SplitString(segment_result_str, ' ', &segs_vec);
...
int32 QueryAutoComplete::IsNormInterrSentence(const string& sent, int32& len) {
...
SentenceType QueryAutoComplete::GetSentenceType(const string &sent) {
  int length = 0;
...
bool QueryAutoComplete::AffirmComplete(const string& query,
...
bool QueryAutoComplete::NegaComplete(const string& query,
                                     const string& last_reply,
                                     string* complete_query) {
  int32 length = 0;
  int32 nega_word_index = IsNormInterrSentence(last_reply, length);
  ...
  if (nega_word_index > 0) {
    ...
    if (!normal_nega_in_head_processed) {
      ...
      string result("");
      segmenter_->SegmentTagging(last_reply, result);
      vector<string> segs;
      SplitString(result, ' ', &segs);

      hash_map<string, pair<string, CompleteType> >::iterator itr =
...
bool QueryAutoComplete::ContainQuery(const string& query, const string& reply) {
  string temp = query;
  vector<string> word_vec;
...
void QueryAutoComplete::RtrimString(const string& str, string* trim_str) {
  vector<string> word_vec;
...
bool QueryAutoComplete::IsPuncEnd(const string& sent) {
  vector<string> word_vec;
  common::CharSegment(sent, word_vec);
  if (word_vec.empty()) {
    return false;
...
void QueryAutoComplete::PopPunc(string& sent, const std::set<string> &punc) {//zhengpeiming
  vector<string> word_vec;
  common::CharSegment(sent, word_vec);
...
void QueryAutoComplete::ReplacePunc(string& sent, //zhengpeiming
                                    const string &spunc, 
                                    const string &dpunc) {
...
bool QueryAutoComplete::JoinQueryReplyComplete(const string& query,
                                               const string& last_reply,
                                               string* complete_query,
                                               bool query_is_front) {
...
bool QueryAutoComplete::JoinQueryQueryComplete(const string& query,
...
CompleteType QueryAutoComplete::Run(const string &query,
                           const string &last_query,
                           const string &last_reply,
                           string* complete_query) {
  int32 completion_src = 0;
  CompleteType complete_ret =
...
CompleteType QueryAutoComplete::Process(const string &query,
                           const string &last_query,
                           const string &last_reply,
                           string* complete_query,
  ...
  if (query.find("？") == string::npos && ContainQuery(query, last_reply_key)) {
    ...
    SentenceType sentence_type = GetSentenceType(last_reply_key);
  ...
  CompleteType complete_type = itr->second.second;
  if (complete_type == AFFIRM_COMPLETE) {
    ...
    *complete_query = last_query;
    ...
    SentenceType sentence_type = GetSentenceType(last_query);
    if (sentence_type == INTERR || sentence_type == NORM_INTERR) {
      *complete_query = last_query;
      completion_src = 1;
    } else {
